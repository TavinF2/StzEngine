...
LRESULT CALLBACK GraphicsSystem::GraphicsSystemMessageHandle(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
 ...
 void GraphicsSystem::StaticInitialize(HWND window, bool fullscreen)
{
 ...
 void GraphicsSystem::Resize(uint32_t width, uint32_t height)
 {
	 ...
	 if(width != GetBackBufferWidth() || height != GetBackBufferHeight())
	 {
		 ...
		 ASSERT(SUCCEEDED(hr), "GraphicsSystem: failed to resize swap chain");
		 mSwapChain->GetDesc(&mSwapChainDesc);
	 }
 ...
 float GraphicsSystem::BackBufferAspectRatio() const
 {
	 return static_cast<float>(GetBackBufferHeight()) / static_cast<float>(GetBackBufferWidth());
 }
